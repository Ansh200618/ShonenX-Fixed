...
class AniyomiBridge(private val context: Context) : MethodChannel.MethodCallHandler {
    ...
    private fun getVideoList(call: MethodCall, result: MethodChannel.Result) {
        ...
        val episode = SEpisode.create().apply {
            ...
            date_upload = (mediaUrl["date_upload"] as? String)?.toLongOrNull()?.takeIf { it > 0 } 
                ?: (mediaUrl["date_upload"] as? Long)?.takeIf { it > 0 } ?: 0L
            episode_number = (mediaUrl["episode_number"] as? String)?.toFloatOrNull()
                ?: (mediaUrl["episode_number"] as? Double)?.toFloat() ?: 0f
            ...
            try {
                ...
                val resultList = res.map { video ->
                    mapOf(
                        ...
                        "quality" to if (video.resolution != null) video.resolution.toString() else video.videoTitle,
    ...
    private fun getPageList(call: MethodCall, result: MethodChannel.Result) {
        ...
        val episode = SChapter.create().apply {
            ...
            date_upload = (mediaUrl["date_upload"] as? String)?.toLongOrNull()?.takeIf { it > 0 } 
                ?: (mediaUrl["date_upload"] as? Long)?.takeIf { it > 0 } ?: 0L
            chapter_number = (mediaUrl["episode_number"] as? String)?.toFloatOrNull()
                ?: (mediaUrl["episode_number"] as? Double)?.toFloat() ?: 0f
            scanlator = mediaUrl["scanlator"] as? String
        }
