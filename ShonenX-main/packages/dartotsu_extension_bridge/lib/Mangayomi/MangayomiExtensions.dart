...
class MangayomiExtensions extends Extension {
  ...
  Future<List<Source>> _fetchAvailable(
    ...
    switch (type) {
      case ItemType.anime:
        activeRepos = List.from(settings.mangayomiAnimeExtensions);
        if (repos != null) {
          activeRepos.addAll(repos.where((r) => !activeRepos.contains(r)));
          settings.mangayomiAnimeExtensions = activeRepos;
        }
        break;
      case ItemType.manga:
        activeRepos = List.from(settings.mangayomiMangaExtensions);
        if (repos != null) {
          activeRepos.addAll(repos.where((r) => !activeRepos.contains(r)));
          settings.mangayomiMangaExtensions = activeRepos;
        }
        break;
      case ItemType.novel:
        activeRepos = List.from(settings.mangayomiNovelExtensions);
        if (repos != null) {
          activeRepos.addAll(repos.where((r) => !activeRepos.contains(r)));
          settings.mangayomiNovelExtensions = activeRepos;
        }
        break;
    }
    isar.writeTxnSync(() => isar.bridgeSettings.putSync(settings));

    final sources = await _manager.fetchAvailableExtensionsStream(type, activeRepos);
