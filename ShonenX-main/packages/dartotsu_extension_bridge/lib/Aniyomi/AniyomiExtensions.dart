...
class AniyomiExtensions extends Extension {
  ...
  Future<List<Source>> _fetchAvailable(
    ...
    switch (type) {
      case ItemType.anime:
        activeRepos = List.from(settings.aniyomiAnimeExtensions);
        if (repos != null) {
          activeRepos.addAll(repos.where((r) => !activeRepos.contains(r)));
          settings.aniyomiAnimeExtensions = activeRepos;
        }
        break;
      case ItemType.manga:
        activeRepos = List.from(settings.aniyomiMangaExtensions);
        if (repos != null) {
          activeRepos.addAll(repos.where((r) => !activeRepos.contains(r)));
          settings.aniyomiMangaExtensions = activeRepos;
        }
        break;
      case ItemType.novel:
        break;
    ...
    final sources = await _loadExtensions(method, repos: activeRepos);
