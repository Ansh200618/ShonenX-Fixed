...
class EpisodeData extends _$EpisodeData {
  ...
  Future<void> changeQuality(int idx) async {
    ...
    final currentSourceHeaders = state.selectedSourceIdx != null ? state.sources[state.selectedSourceIdx!].headers : null;
    final resolvedHeaders = currentSourceHeaders?.isNotEmpty == true ? currentSourceHeaders : state.headers;

    _player.open(
      url,
      ref.read(playerStateProvider).position,
      headers: resolvedHeaders?.cast<String, String>(),
    );
  }
  ...
  Future<void> _loadSourceStream(
    ...
    final resolvedHeaders = src.headers?.isNotEmpty == true ? src.headers : state.headers;

    final qualities = await _getQualities(src, resolvedHeaders);
  ...
  Future<BaseSourcesModel?> _fetchSourceData(
    ...
    if (_exp.useExtensions && ep.url != null) {
      ...
      return BaseSourcesModel(
        headers: res.firstOrNull?.headers?.cast<String, String>(),
        sources: res
            .map(
              (s) => Source(
                url: s?.url,
                ...
                headers: s?.headers?.cast<String, String>(),
              ),
            )
            .toList(),
        tracks:
