...
class _DownloadSourceSelectorState extends State<DownloadSourceSelector> {
  ...
  Future<void> _init() async {
    ...
    try {
      ...
      setState(() {
        ...
        _globalHeaders = data.headers?.cast<String, String>();
        _loading = false;
  ...
  Future<void> _expandSource(int index, Source source) async {
    ...
    try {
      ...
      final resolvedHeaders = source.headers?.isNotEmpty == true ? source.headers : _globalHeaders;

      List<Map<String, dynamic>> result = [];
      if (source.isM3U8) {
        result = await extractor
            .extractQualities(url, resolvedHeaders ?? {}, source.isM3U8)
